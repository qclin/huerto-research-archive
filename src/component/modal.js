...
function PreviewModal({ preview, onClose }) {
  ...
  const RenderLink = () => (
    medialURL && 
      <a href={medialURL} target="_blank" rel="noreferrer" className="underline">Link</a>
  ) 

  return (
    <Modal isOpen={!!preview} onRequestClose={onClose} style={customStyle(CATEGORY[0].toLowerCase())}>
      ...
      {isImage ? (
        <figure key={`${CATEGORY}.${IDENTIFIER}`}>
          ...
          <figcaption className="flex items-center text-xs mt-1 justify-between">
            <p>
              {CATEGORY.map((value) => (<span className="rounded-full border-white border p-1 px-2 mx-1">{value}</span>))}
              <span className="uppercase">{MEDIA_TYPE}</span> / {[TITLE,YEAR].join(' / ')}
            </p>
            {RenderLink()}
            </figcaption>
        </figure>
