...
function PlotRow({items, className, images, setPreview, selectedCategory}){
  ...
  const RecipeCard = ({item, index, isFocus}) => {
  ...
  return (
    <div className={clsx(className, "flex my-4 items-center max-h-25vh 2xl:max-h-30vh")}>
      {items.map((item, index) => {
          ...
          if(isRecipe) return <RecipeCard key={`recipe-${index}`} item={item} index={index} isFocus={isFocus}/>
          if(!imageData) return  <Placeholder key={`placeholder-${index}`} item={item} index={index} isFocus={isFocus}/>
