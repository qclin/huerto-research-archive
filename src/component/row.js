...
function Row({fields, type}){
  ...
  return (
    ...
    <dt key={type} className="border-r absolute flex-shrink-0 w-32 left-0 " >
      ...
      <ArrowButton Arrow={PrevArrow} onClick={() => sliderRef.current.slickPrev()}/>
      <ArrowButton Arrow={NextArrow} onClick={() => sliderRef.current.slickNext()}/>
    </dt>
    <Slider {...settings} ref={sliderRef} className="h-64 inline ml-32" style={{width: "calc(100vw - 8rem)"}}>
